...
static ICall_EntityID  zclSampleTemperatureSensor_Entity;
...
static void zclSampleTemperatureSensor_changeKeyCallback(uint8_t keysPressed);
static void zclSampleTemperatureSensor_processKey(uint8 keysPressed);
...
static uint8 zclSampleTemperatureSensor_ProcessIncomingMsg( zclIncoming_t *msg );
#ifdef ZCL_READ
static uint8 zclSampleTemperatureSensor_ProcessInReadRspCmd( zclIncoming_t *pInMsg );
#endif
#ifdef ZCL_WRITE
static uint8 zclSampleTemperatureSensor_ProcessInWriteRspCmd( zclIncoming_t *pInMsg );
#endif
static uint8 zclSampleTemperatureSensor_ProcessInDefaultRspCmd( zclIncoming_t *pInMsg );
#ifdef ZCL_DISCOVER
static uint8 zclSampleTemperatureSensor_ProcessInDiscCmdsRspCmd( zclIncoming_t *pInMsg );
static uint8 zclSampleTemperatureSensor_ProcessInDiscAttrsRspCmd( zclIncoming_t *pInMsg );
static uint8 zclSampleTemperatureSensor_ProcessInDiscAttrsExtRspCmd( zclIncoming_t *pInMsg );
#endif // ZCL_DISCOVER
...
ZStatus_t batt_ReadWriteAttrCB( uint16 clusterId, uint16 attrId, uint8 oper,
...
/*******************************************************************************
 * @fn          zclSampleTemperatureSensor_task
 *
 * @brief       Application task entry point for the ZStack Sample Temperature Sensor
 *              Application
 *
...
void zclSampleTemperatureSensor_task(NVINTF_nvFuncts_t *pfnNV)
{
...
static void zclSampleTemperatureSensor_initialization(void)
{
    ...
    /* Initialize keys */
    Board_Key_initialize(zclSampleTemperatureSensor_changeKeyCallback);
...
static void SetupZStackCallbacks(void)
{
    ...
    (void)Zstackapi_DevZDOCBReq(zclSampleTemperatureSensor_Entity, &zdoCBReq);
}
...
static void zclSampleTemperatureSensor_Init( void )
{
  ...
  zclport_registerEndpoint(zclSampleTemperatureSensor_Entity, &zclSampleTemperatureSensorEpDesc);
...
/*  Req.attrID = ATTRID_MS_TEMPERATURE_MEASURED_VALUE;
  ...
  Zstackapi_bdbRepAddAttrCfgRecordDefaultToListReq(zclSampleTemperatureSensor_Entity,&Req);

  zcl_registerReadWriteCB(SAMPLETEMPERATURESENSOR_ENDPOINT,batt_ReadWriteAttrCB,NULL);


   Req1.attrID = ATTRID_TEST_SENSOR;
   Req1.cluster = ZCL_CLUSTER_ID_TEST;
   Req1.endpoint = SAMPLETEMPERATURESENSOR_ENDPOINT;
   Req1.maxReportInt = 10;
   Req1.minReportInt = 0;
   memcpy(Req1.reportableChange,reportableChange,BDBREPORTING_MAX_ANALOG_ATTR_SIZE);

   Zstackapi_bdbRepAddAttrCfgRecordDefaultToListReq(zclSampleTemperatureSensor_Entity,&Req1);
...
*/
  ...
  UI_Init(zclSampleTemperatureSensor_Entity, &events, sem, SAMPLEAPP_LCD_AUTO_UPDATE_EVT, SAMPLEAPP_PROCESS_UI_UART_EVT, &zclSampleTemperatureSensor_IdentifyTime, APP_TITLE, &zclSampleTemperatureSensor_uiAppUpdateLcd, zclSampleTemperatureSensor_uiAppStatesMain);

#if defined ( BDB_TL_INITIATOR )
    touchLinkInitiatorApp_Init(zclSampleTemperatureSensor_Entity);
#elif defined ( BDB_TL_TARGET )
    touchLinkTargetApp_Init(zclSampleTemperatureSensor_Entity);
#endif
...
static void zclSampleTemperatureSensor_initParameters(void)
{
    ...
    Zstackapi_bdbSetAttributesReq(zclSampleTemperatureSensor_Entity, &zstack_bdbSetAttrReq);
}
        ...
        if(ICall_wait(ICALL_TIMEOUT_FOREVER) == ICALL_ERRNO_SUCCESS)
        {
               ...
               == ICALL_ERRNO_SUCCESS)
            {
                if( (stackid == ICALL_SERVICE_CLASS_ZSTACK)
                    && (dest == zclSampleTemperatureSensor_Entity) )
                {
                    if(pMsg)
                    {
            ...
            {
                // Process Key Presses
                zclSampleTemperatureSensor_processKey(keys);
                keys = 0;
            ...
            if ( events & SAMPLEAPP_END_DEVICE_REJOIN_EVT )
            {
              ...
              Zstackapi_bdbZedAttemptRecoverNwkReq(zclSampleTemperatureSensor_Entity,&zstack_bdbZedAttemptRecoverNwkRsp);
...
static void zclSampleTemperatureSensor_processZStackMsgs(zstackmsg_genericReq_t *pMsg)
{
      switch(pMsg->hdr.event)
      {
          ...
          case zstackmsg_CmdIDs_BDB_CBKE_TC_LINK_KEY_EXCHANGE_IND:
          {
            ...
            Zstackapi_bdbCBKETCLinkKeyExchangeAttemptReq(zclSampleTemperatureSensor_Entity, 
          ...
          case zstackmsg_CmdIDs_BDB_FILTER_NWK_DESCRIPTOR_IND:
            ...
            */
            ...
            Zstackapi_bdbFilterNwkDescComplete(zclSampleTemperatureSensor_Entity);
          break;
              ...
              if(pInd->req.state==zstack_DevState_DEV_END_DEVICE)
                  ...
                  zAddrType_t dstAddr;
                    ...
                    ZDP_EndDeviceBindReq( &dstAddr, NLME_GetShortAddr(),
                        ...
                        zclSampleTemperatureSensor_SimpleDesc.AppProfId,
                        zclSampleTemperatureSensor_SimpleDesc.AppNumInClusters, (cId_t *)zclSampleTemperatureSensor_SimpleDesc.pAppInClusterList,
                        zclSampleTemperatureSensor_SimpleDesc.AppNumOutClusters, (cId_t *)zclSampleTemperatureSensor_SimpleDesc.pAppOutClusterList,
                        FALSE );
    ...
    switch(bdbCommissioningModeMsg->bdbCommissioningMode)
    {
      case BDB_COMMISSIONING_FORMATION:
        if(bdbCommissioningModeMsg->bdbCommissioningStatus == BDB_COMMISSIONING_SUCCESS)
        {
          ...
          Zstackapi_bdbStartCommissioningReq(zclSampleTemperatureSensor_Entity,&zstack_bdbStartCommissioningReq);
 ...
 */
static uint8 zclSampleTemperatureSensor_ProcessIncomingMsg( zclIncoming_t *pInMsg)
{
  uint8 handled = FALSE;
...
/*********************************************************************
 * @fn      zclSampleTemperatureSensor_changeKeyCallback
 *
...
static void zclSampleTemperatureSensor_processKey(uint8 keysPressed)
{
  ...
  if(keysPressed == KEY_RIGHT)
  {
             ...
             zclReportCmd_t *Sensorcmd;
             Sensorcmd = osal_mem_alloc( sizeof(zclReportCmd_t) + sizeof(zclReport_t) );
             if ( Sensorcmd != NULL )
             {
             Sensorcmd->numAttr=1;
             ...
             Sensorcmd->attrList[0].attrData=(void *)&zclSampleTemperatureSensor_battRemaining;


             zcl_SendReportCmd(  SAMPLETEMPERATURESENSOR_ENDPOINT,  &sensordstAddr,
                             ZCL_CLUSTER_ID_TEST, Sensorcmd,
                             ZCL_FRAME_SERVER_CLIENT_DIR,1,
...
static void zclSampleTemperatureSensor_UiActionChangeTemp(uint16 keys)
{
  ...
  if ( keys & UI_KEY_SW_1_PRESSED )
  {
    ...
    if ( zclSampleTemperatureSensor_MeasuredValue < zclSampleTemperatureSensor_MaxMeasuredValue )
    {
      ...
      Zstackapi_bdbRepChangedAttrValueReq(zclSampleTemperatureSensor_Entity,&Req);
#endif
    }
